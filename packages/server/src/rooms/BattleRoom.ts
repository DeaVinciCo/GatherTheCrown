...
export class BattleRoom extends Room<BattleState> {
  ...
  private readonly winsToClinch = Math.ceil(this.maxRounds / 2);

  onCreate() {
    this.setState({ players: {}, round: 0, timer: 0, status: 'waiting', roundHistory: [] });
    ...
    this.onMessage('hit', (client, message: ClientMessage) => {
      ...
      opponent.hp -= dmg;
      if (opponent.hp <= 0) {
        this.endRound(client.sessionId, 'ko');
      }
    });
  }
  ...
  onLeave(client: Client) {
    const remainingIds = Object.keys(this.state.players).filter((id) => id !== client.sessionId);
    delete this.state.players[client.sessionId];

    if (this.state.status === 'fighting' || this.state.status === 'countdown') {
      const remaining = remainingIds[0];
      this.endRound(remaining, 'forfeit');
    }
  ...
  private endRound(winnerId: string | undefined, reason: RoundResult['reason']) {
    if (winnerId) {
    ...
    const matchOver =
      ...
      maxWins >= this.winsToClinch;

    if (matchOver) {
      const sorted = players.sort(
        (a, b) => this.state.players[b].wins - this.state.players[a].wins
      );
