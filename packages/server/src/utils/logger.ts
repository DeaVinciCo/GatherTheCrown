...
const parseConfiguredTransports = (value: string | undefined): Set<Transport> => {
  ...
  for (const transport of configured) {
    if (!VALID_TRANSPORTS.has(transport as Transport)) {
      console.warn(`[LOGGER] Ignoring unsupported LOG_TRANSPORT value: "${transport}"`);
      continue;
    }

    transports.add(transport as Transport);
  }
